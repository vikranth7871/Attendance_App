...
import { Award, Download, Calendar, Clock, MapPin, TrendingUp, Printer } from 'lucide-react';
...
const StudentResults = () => {
    ...
    const tabBtn = (label, active, count, color='var(--brand-primary)', shadow='rgba(99,102,241,0.35)') => (
        <button onClick={() => active !== label && (setActiveTab ? setActiveTab(label) : null)}
            style={{ padding:'0.4rem 1rem', borderRadius:'999px', border:'none', cursor:'pointer', fontWeight:'700', fontSize:'0.82rem', transition:'all 0.2s',
                background: active === label ? color : 'var(--bg-secondary)',
                color: active === label ? '#fff' : 'var(--text-secondary)',
                boxShadow: active === label ? `0 2px 8px ${shadow}` : 'none' }}>
            {label}{count != null && <span style={{ marginLeft:'0.4rem', background: active===label?'rgba(255,255,255,0.25)':'var(--border-color)', padding:'0.05rem 0.4rem', borderRadius:'999px', fontSize:'0.7rem' }}>{count}</span>}
        </button>
    );
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem' }}>
                ...
                <button
                    ...
                    style={{ display:'flex', alignItems:'center', gap:'0.5rem', padding:'0.75rem 1.25rem', borderRadius:'0.85rem',
                        fontWeight:'700', background:'var(--brand-primary)', color:'#fff', border:'none',
                        cursor: results.length ? 'pointer' : 'not-allowed', opacity: results.length ? 1 : 0.5, fontSize:'0.9rem' }}>
            ...
            <div className="glass-panel" style={{ padding:'1.5rem', display:'flex', flexDirection:'column', gap:'1.25rem' }}>
                ...
                <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap', borderBottom:'1px solid var(--border-color)', paddingBottom:'0.75rem' }}>
                    <button onClick={() => setScheduleTab('upcoming')}
                        style={{ padding:'0.4rem 1rem', borderRadius:'999px', border:'none', cursor:'pointer', fontWeight:'700', fontSize:'0.82rem', transition:'all 0.2s',
                            background: scheduleTab==='upcoming'?'#10b981':'var(--bg-secondary)',
                            color: scheduleTab==='upcoming'?'#fff':'var(--text-secondary)',
                            boxShadow: scheduleTab==='upcoming'?'0 2px 8px rgba(16,185,129,0.35)':'none' }}>
                    ...
                    {scheduleTerms.map(term => (
                        <button key={term} onClick={() => setScheduleTab(term)}
                            style={{ padding:'0.4rem 1rem', borderRadius:'999px', border:'none', cursor:'pointer', fontWeight:'700', fontSize:'0.82rem', transition:'all 0.2s',
                                background: scheduleTab===term?'var(--brand-primary)':'var(--bg-secondary)',
                                color: scheduleTab===term?'#fff':'var(--text-secondary)',
                                boxShadow: scheduleTab===term?'0 2px 8px rgba(99,102,241,0.35)':'none' }}>
                            {term}
                        </button>
                    ))}
                </div>
                    ...
                    <motion.div key={scheduleTab} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-8 }} transition={{ duration:0.18 }}
                        ...
                        {(() => {
                            ...
                            return list.map((sc, i) => {
                                ...
                                return (
                                    <div key={sc.id||i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'0.75rem',
                                        padding:'1rem 1.25rem', borderRadius:'12px', background:'var(--bg-secondary)',
                                        border:`1px solid ${isUrgent?'rgba(245,158,11,0.35)':'var(--border-color)'}`,
                                        borderLeft:`3px solid ${isUrgent?'#f59e0b':expired?'#94a3b8':'var(--brand-primary)'}`,
                                        ...
                                        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'0.35rem' }}>
                                            <span style={{ padding:'0.25rem 0.75rem', borderRadius:'999px', fontSize:'0.75rem', fontWeight:'800',
                                                background: isUrgent?'rgba(245,158,11,0.12)':expired?'rgba(148,163,184,0.12)':'rgba(99,102,241,0.1)',
                                                color: isUrgent?'#f59e0b':expired?'#94a3b8':'#818cf8' }}>
