...
import {
  ...
  TextInput, RefreshControl, Alert, Switch
} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import {
  Award, Sparkles, Plus, Search, Trash2, Clock, Gamepad2, X
...
const TeacherQuizManageScreen = ({ navigation }) => {
  ...
  const handleTogglePublish = async (quiz) => {
    try {
      ...
      Alert.alert('Error', err.response?.data?.message || 'Failed to toggle publish status.');
    }
  };
  ...
  const handleAIGenerated = async (generatedData) => {
    try {
      ...
      Alert.alert('Save Error', err.response?.data?.message || 'Could not save AI quiz.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <Header
        title="My Quizzes"
        subtitle={`${quizzes.length} quizzes created`}
        rightAction={
        ...
        <FlatList
            ...
            <View style={[styles.card, shadows.sm]}>
              <View style={styles.cardHeader}>
                <View style={[styles.typeBadge, { backgroundColor: colors.teacher + '22' }]}>
                  <Text style={[styles.typeBadgeText, { color: colors.teacher }]}>
                    {item.type || 'Practice'}
                  </Text>
                </View>
              ...
              <View style={styles.metaRow}>
                <View style={styles.metaItem}>
                  ...
                  <Text style={styles.metaText}>{item.questionCount || item.questions?.length || 0} Questions</Text>
                </View>
                <View style={styles.metaItem}>
                  <Clock size={13} color={colors.textMuted} />
                  <Text style={styles.metaText}>{item.timeLimit || 30} mins</Text>
                </View>
              </View>
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptySub}>Tap 'AI Quiz' to generate practice questions for your class</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  aiBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: colors.teacher, paddingHorizontal: 12, paddingVertical: 7, borderRadius: radius.full },
...
export default TeacherQuizManageScreen;
